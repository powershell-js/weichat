...
Page({
  ...
  submit: function () {
    ...
    if (title == "停车明细") {
      wx.navigateTo({
        url: '../dynamics/dynamics?token=' + this.data.token + "&startTime=" + this.data.beginTime + "&endTime=" + this.data.endTime + "&vehicleId=" + this.data.vehicleId + "&timeInterval=" + this.data.interval,
      })
    }
