...
Page({
  ...
  allId: function() {
    var that = this
    wx.request({
      url: 'https://www.123gps.com.cn/app/vehicleSingleList',
      data:{
  ...
  submit: function() {
    ...
    console.log(this.data.token, this.data.beginTime, this.data.endTime, this.data.vehicleId, this.data.interval)
        ...
        if (title == "温度明细") {
          wx.navigateTo({
            url: '../Temperature/Temperature?token=' + this.data.token + "&startTime=" + this.data.beginTime + "&endTime=" + this.data.endTime + "&vehicleId=" + this.data.vehicleId + "&timeInterval=" + this.data.interval,
          })
        }
